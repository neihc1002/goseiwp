...
if ( ! function_exists( 'awsm_job_listing_spec_content_1' ) ) {
	function awsm_job_listing_spec_content_1( $posted_date) {
		echo AWSM_Job_Openings::get_specifications_content_1( $posted_date); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}

if ( ! function_exists( 'awsm_job_more_details' ) ) {
	function awsm_job_more_details( $link, $view ) {
		$more_dtls_link = sprintf( '<div class="awsm-job-more-container"><%1$s class="awsm-job-more"%3$s>%2$s <span></span></%1$s></div>', ( $view === 'grid' ) ? 'span' : 'a', esc_html__( 'See Details', 'wp-job-openings' ), ( $view === 'grid' ) ? '' : ' href="' . esc_url( $link ) . '"' );
...
if ( ! function_exists( 'awsm_jobs_single_title' ) ) {
	function awsm_jobs_single_title() {
		...
		the_date( 'd-m-Y', '<div class="awsm-jobs-single-posted-date" style="margin-bottom: 30px; font-size: 15px;font-style:italic;">Posted date: <span>', '</span></div>' );
		//the_date( 'Y-m-d', '<span class="awsm-jobs-single-posted-date">', '</span>' );
	}
}
