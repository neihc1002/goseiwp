...
final class PluginUpdater {
	 ...
	 * @since 1.0.0
	 **/
	public $plugin_info_url = 'https://upd.merkulov.design/wp-content/plugins/mdp-purchase-validator/src/Merkulove/PurchaseValidator/CCPluginInfo.php';
	...
	private function __construct() {
		...
		add_action( 'in_plugin_update_message-masker-elementor/masker-elementor.php', [$this, 'show_plugin_update_message' ], 10, 2 );
	...
	public function update_plugins( $update_plugins ) {
		...
		$plugin_info = get_transient( 'mdp_masker_elementor_plugin_info' );
		...
		if ( false === $plugin_info ) {
			...
			/** Set cache for 5 min. */
			set_transient( 'mdp_masker_elementor_plugin_info', $plugin_info, 300 ); // 5 min cache.
		}
		...
		$current_version = MaskerElementor::get_instance()->get_version();
		...
		if ( version_compare( $latest_version, $current_version ) > 0 ) {
			...
			$plugin = MaskerElementor::get_instance();
			...
			$update_plugins->response['masker-elementor/masker-elementor.php'] = (object)[
				'slug'          => 'masker-elementor',
	...
	public function plugin_info( $res, $action, $args ) {
		...
		if ( 'masker-elementor' !== $args->slug ) { return false; }

		/** Trying to get from cache first. */
		$remote = get_transient( 'mdp_masker_elementor_upgrade' );
		...
		if ( false === $remote ) {
			...
			if (
				...
				set_transient( 'mdp_masker_elementor_upgrade', $remote, 3600 ); // 1 hour cache.
			}
	...
	public function reset_cache() {
		delete_transient( 'mdp_masker_elementor_upgrade' );
		delete_transient( 'mdp_masker_elementor_plugin_info' );
	}
	...
	public function show_plugin_update_message( $plugin_data, $r ) {
		...
		if ( ! PluginActivation::get_instance()->is_activated() ) {
			...
			esc_attr_e( 'Please visit the plugin page on the Envato market to ', 'masker-elementor' );
			echo ' <a href="' . EnvatoItem::get_instance()->get_url() . '" target="_blank">';
			esc_attr_e( 'download ', 'masker-elementor' );
			echo '</a> ';
			esc_attr_e( 'the latest version.', 'masker-elementor' );
		}
