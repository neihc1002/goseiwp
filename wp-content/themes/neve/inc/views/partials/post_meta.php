...
class Post_Meta extends Base_View {
		...
		foreach ( $order as $meta ) {
			switch ( $meta ) {
				...
				case 'reading':
					...
					break;
				case 'views':
					$markup .= '<' . $tag . ' class="meta views">';
					$markup .= self::get_views();
					$markup .= '</' . $tag . '>';
					break;
				case 'default':
				default:
	...
	public static function get_views() {
		global $wpdb;
		$pid = get_the_ID();
		$raw_stats = $wpdb->get_var( $wpdb->prepare( "SELECT all_time_stats FROM {$wpdb->prefix}most_popular WHERE post_id = '%d'", array( $pid ) ) );
		$text = '%s View';
		if($raw_stats==''){
			$raw_stats = 0;
		}
		...
		/* translators: %s: number of comments */
		$comments = sprintf( _n( $text, $text, $raw_stats, 'neve' ), $raw_stats );

		return esc_html( $comments );
	}
